import { Baby, Activity, Moon, Bell, Settings, LogOut, Video } from 'lucide-react';
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center space-x-3">
            <div className={`p-2 rounded-xl transition-colors duration-200 ${
              ...
              <Baby className={`w-6 h-6 transition-colors duration-200 ${
                isDarkMode ? 'text-rose-300' : 'text-rose-500'
              }`} />
            </div>
            <h1 className={`text-2xl font-semibold bg-clip-text text-transparent transition-all duration-200 ${
              isDarkMode 
                ...
                {user?.picture && (
                  <img
                    ...
                    className={`w-8 h-8 rounded-full border-2 transition-colors duration-200 ${
                      isDarkMode ? 'border-rose-700' : 'border-rose-200'
                    }`}
                  />
                )}
                <span className={`text-sm font-medium hidden sm:inline transition-colors duration-200 ${
              ...
              {showUserMenu && (
                <div className={`absolute right-0 mt-2 w-48 rounded-lg shadow-lg border z-50 transition-colors duration-200 ${
                  ...
                  <div className={`p-4 border-b transition-colors duration-200 ${
                    isDarkMode ? 'border-slate-700' : 'border-rose-50'
                  }`}>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center space-x-2 px-4 py-3 text-sm font-medium transition-all cursor-pointer ${
                      isDarkMode
